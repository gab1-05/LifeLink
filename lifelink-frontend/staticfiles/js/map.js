...
function initMap() {
    ...
    try {
        ...
        // Viewport based fetching for hospitals
...
async function loadHospitalsInView() {
    ...
    try {
        hospitalData = await api(`hospitals-in-view/?south=${south}&west=${west}&north=${north}&east=${east}`);
        renderHospitals();
    } catch (e) {
        console.error('Failed to load viewport hospitals', e);
    }
}
...
function renderHospitals() {
    ...
    hospitalData.forEach(h => {
        ...
        let popupHtml = `
            <div class="popup-title">${h.name || 'Hospital'}</div>
            <div class="popup-row" style="margin-bottom:12px;"><span>${h.address || 'No address provided'}</span></div>
...
function requestBloodAtHospital(name, address, lat, lng) {
    // Attempt to open the request modal if on a page that supports it (like dashboard)
    if (typeof window.openModal === 'function' && document.getElementById('req-hospital')) {
        document.getElementById('req-hospital').value = name;
        if (document.getElementById('req-address')) document.getElementById('req-address').value = address;
        if (typeof window.setRequestLocation === 'function') window.setRequestLocation(lat, lng, name);
        window.openModal('request-modal');
    } else {
        // Fallback: Redirect to requests page with query params
        window.location.href = `/requests/?hosp_name=${encodeURIComponent(name)}&hosp_addr=${encodeURIComponent(address)}&lat=${lat}&lng=${lng}#new`;
    }
}

function toggleLayer(layerName, element) {
    let layer;
    if (layerName === 'donor') layer = donorLayer;
    else if (layerName === 'request') layer = requestLayer;
    else if (layerName === 'hospital') layer = hospitalLayer;
